...
void ModelComponent::SetPosition(float x, float y, float z)
{
	modelPosition.x = x;
	modelPosition.y = y;
	modelPosition.z = z;
...
XMFLOAT3 ModelComponent::GetPosition()
{
	return modelPosition;
}

void ModelComponent::SetDiffuseColor(float x, float y, float z, float w)
{
...
XMFLOAT4 ModelComponent::GetDiffuseColor()
{
	return modelDiffustColor;
}

void ModelComponent::SetExtension(WCHAR * extension)
{
	// 파일 확장자 셋팅
...
ShaderManager::ShaderType ModelComponent::GetShaderType()
{
	return m_shaderType;
}

bool ModelComponent::CheckAndModelLoader(WCHAR * modelFileName)
{
	// 파일 있는 지 여부 체크
	struct stat buffer;
...
bool ModelComponent::ModelLoader(WCHAR * modelFileName)
{
	// 파일 읽기
...
bool ModelComponent::InitializeBuffers(ID3D11Device* device, WCHAR* modelFileName)
{
...
bool ModelComponent::LoadTexture(ID3D11Device* device, WCHAR * textureFilename)
{
